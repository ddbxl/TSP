...
/* A worker that runs out of memory dies without firing onerror in every
   ...
   reports progress as it goes, so only the quiet requests are timed. */
const PATIENCE = { deliver: 300000, html: 300000, text: 120000, boot: 300000 };
const FOREVER = new Set(["process", "applyOcr", "inspect"]);
...
function ask(type, payload = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!FOREVER.has(type)) {
      const limit = PATIENCE[type] || 120000;
      setTimeout(() => {
        if (pending.has(id)) {
          settle(
            id,
            new Error(
              `the engine did not answer the ${type} request within ` +
                `${Math.round(limit / 1000)} seconds, which usually means it ran ` +
                `out of memory on a large batch`
            ),
            true
          );
        }
...
/* The engine arrives in three stages, so the bar tracks something real. Which
   component is downloading is the engine's business; the wait is the reader's.
   The failure panel carries the versions, where they matter. */
let bootStep = 0;
...
function setEngine(state, text) {
  ...
  ui.engine.hidden = state === "ready";

  if (state === "loading") {
    bootStep = Math.min(bootStep + 1, 3);
    ui.engineFill.style.width = `${[0, 30, 72, 92][bootStep]}%`;
    ui.engineText.textContent = "Starting the engine";
  } else if (state === "ready") {
    ui.engineFill.style.width = "100%";
    ui.engineText.textContent = "Ready";
  } else if (state === "failed") {
    ui.engineFill.style.width = "100%";
    ui.engineText.textContent = "The engine did not start";
  } else {
    bootStep = 0;
    ui.engineFill.style.width = "0%";
...
function rowActions(entry) {
  ...
  page.type = "button";
  page.className = "link";
  page.textContent = "Download HTML";
  page.addEventListener("click", async () => {
    try {
      const answer = await ask("html", { name: entry.file.name });
      saveBlob(
        new Blob([answer.html], { type: "text/html;charset=utf-8" }),
        optimisedName(entry.file.name, "html")
      );
    } catch (error) {
      log(`${entry.file.name}: ${describe(error)}`);
    }
...
async function offerResults() {
  ...
  // One page holding every document. Built when it is asked for: a batch of
  // hundreds costs real memory to assemble, and most runs never want it.
  ui.downloadHtml.hidden = false;
  ui.downloadHtml.textContent = "Download all as HTML";
  ui.downloadHtml.dataset.name = single
    ? optimisedName(single, "html")
    : "optimised_documents.html";
...
/* A switch is on or off, so there is no explicit "follow the system" here. It
   starts wherever the system says and remembers the first time it is touched. */
...
ui.theme.addEventListener("change", () => {
  ...
  try {
    ...
    // Storage turned off still leaves the choice standing for this visit.
  }
});
...
ui.downloadHtml.addEventListener("click", async (event) => {
  event.preventDefault();
  const label = ui.downloadHtml.textContent;
  ui.downloadHtml.textContent = "Building the page";
  try {
    const page = await ask("html");
    if (!page.html) {
      log("Nothing to put in an HTML page yet.");
      return;
    }
    saveBlob(
      new Blob([page.html], { type: "text/html;charset=utf-8" }),
      ui.downloadHtml.dataset.name || "optimised_documents.html"
    );
    log(`HTML page ready, ${humanSize(page.html.length)}.`);
  } catch (error) {
    log(`HTML could not be prepared: ${describe(error)}`);
  } finally {
    ui.downloadHtml.textContent = label;
  }
});
