...
def _bundle(name=None):
    """Every file the output should contain, as (path in the bundle, payload).

    Collecting 357 markdown files meant opening 357 folders, so they gather in
    one place, the HTML beside them, and the full per-document folders under
    `documents/`. The HTML in the bundle points at the pictures where they lie,
    so nothing is stored twice and it works once extracted.

    The zip and the folder save both read this, which keeps them agreeing.
    """
    from tsp.render import to_fragment, wrap
    ...
    for folder in _folders(name):
        ...
        for page in sorted(folder.glob("*.md")):
            body = page.read_text(encoding="utf-8")
            items.append((f"markdown/{page.name}", body.encode("utf-8")))
            fragment = to_fragment(body, link=f"../documents/{folder.name}/")
            items.append((
                f"html/{page.stem}.html",
                wrap(fragment, page.stem).encode("utf-8"),
            ))
    ...
    for path in sorted(root.rglob("*")):
        ...
        if only and relative.parts[0] != only:
            continue
        items.append((f"documents/{relative.as_posix()}", path))

    return items


def tsp_zip(name=None):
    """One archive: the markdown together, the HTML together, and the full
    folders behind them."""
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        for arcname, payload in _bundle(name):
            if isinstance(payload, bytes):
                archive.writestr(arcname, payload)
            else:
                archive.write(payload, arcname)
...
def tsp_html(name=None):
    """One document as HTML, or every one of them in a single page.

    Built on request from the markdown, so it needs no setting turned on
    beforehand. Each document is converted against its own folder, because page
    images are numbered per document and `p001.png` exists in all of them.
    """
    ...
    for folder in _folders(name):
        for page in sorted(folder.glob("*.md")):
            fragments.append(
                to_fragment(page.read_text(encoding="utf-8"), folder, budget)
            )
            titles.append(page.stem)
    if not fragments:
        return ""
    title = titles[0] if len(titles) == 1 else f"{len(titles)} documents"
    return wrap(fragments, title)


def tsp_files(name=None):
    return json.dumps([arcname for arcname, _ in _bundle(name)])


def tsp_read(relative):
    """One file out of the bundle, for saving to a folder."""
    for arcname, payload in _bundle():
        if arcname == relative:
            return payload if isinstance(payload, bytes) else payload.read_bytes()
    # An OCR step asks for a page image by its own path.
