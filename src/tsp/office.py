...
OFFICE_SUFFIXES = {".docx", ".odt", ".pptx", ".odp"}

# A cell longer than this holds prose, so the table is a layout frame rather
# than data. Measured on one long agreement: data cells reached 143
# characters, layout cells 5,000 to 59,000.
MAX_DATA_CELL = 300
...
def _docx_styles(archive) -> tuple[dict[str, int], set[str]]:
    ...
    One language's headings are called Naslov1 and Naslov2, a German's
...
def _shape_text(shape) -> list[tuple[str, bool]]:
    """Each paragraph in a shape, with whether it sits at an outline level."""
    out = []
    for paragraph in shape.iter(f"{A_NS}p"):
        text = "".join(run.text or "" for run in paragraph.iter(f"{A_NS}t")).strip()
        if not text:
            continue
        properties = paragraph.find(f"{A_NS}pPr")
        level = properties is not None and properties.get("lvl") is not None
        out.append((text, level))
    return out


def _is_title(shape) -> bool:
    holder = shape.find(f".//{P_NS}ph")
    return holder is not None and (holder.get("type") or "") in ("title", "ctrTitle")


def _read_pptx(path: Path) -> list[Block]:
    blocks: list[Block] = []
    with zipfile.ZipFile(path) as archive:
        slides = sorted(
        ...
        for position, slide in enumerate(slides, start=1):
            ...
            for shape in root.iter(f"{P_NS}sp"):
                ...
                if not lines:
                    continue
                if _is_title(shape) and not titles:
                    titles.extend(text for text, _ in lines)
                else:
                    body.extend(lines)
            ...
            for table in root.iter(f"{A_NS}tbl"):
                ...
                for row in table.findall(f"{A_NS}tr"):
                    rows.append([
                        " ".join(
                            run.text or "" for run in cell.iter(f"{A_NS}t")
                        ).strip()
            ...
            heading = titles[0] if titles else f"Slide {position}"
            blocks.append(Block("heading", f"Slide {position}: {heading}", level=2))
            for extra in titles[1:]:
                blocks.append(Block("body", extra))
            for text, bullet in body:
                blocks.append(Block("list" if bullet else "body", text))
            for rows in grids:
                blocks.append(Block("table", rows=rows))

            part = notes.get(position)
            if part:
                spoken = " ".join(
                    (run.text or "")
                    for run in ET.fromstring(archive.read(part)).iter(f"{A_NS}t")
                ).strip()
                # Slide numbers reappear here as a lone digit; nobody wants that.
                if spoken and spoken != str(position):
                    blocks.append(Block("body", f"Speaker notes: {spoken}"))

    return blocks


def _read_odp(path: Path) -> list[Block]:
    with zipfile.ZipFile(path) as archive:
        root = ET.fromstring(archive.read("content.xml"))
    body = root.find(f"{OFFICE_NS}body")
    show = body.find(f"{OFFICE_NS}presentation") if body is not None else None
    blocks: list[Block] = []
    if show is None:
        return blocks

    for position, page in enumerate(show.findall(f"{DRAW_NS}page"), start=1):
        name = page.get(f"{DRAW_NS}name") or f"Slide {position}"
        blocks.append(Block("heading", f"Slide {position}: {name}", level=2))
        for frame in page.iter(f"{DRAW_NS}frame"):
            for paragraph in frame.iter(f"{TEXT_NS}p"):
                text = _odt_text(paragraph)
                if text:
                    blocks.append(Block("body", text))
    return blocks
...
def read_office(path: str | Path) -> list[Block]:
    """Read a Word, OpenDocument, PowerPoint or Impress file into blocks."""
    path = Path(path)
    ...
    raise ValueError(f"TSP has no reader for {path.suffix}: {path.name}")
