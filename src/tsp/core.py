...
class Settings:
    ...
    # long report, boxes and charts 50 to 100%.
    ...
    # chart cannot be told from a sparse table by any measure of its text: both
    # run 1.4 to 2.0 words a line. With
...
def _chart_regions(page, boxes, blocks, keep_as_text, settings: Settings) -> list:
    ...
    A chart's labels arrive as orphan fragments: measured on a long report, 1.5 to 4 words a line against 9 to 12 inside a bordered box of
...
OFFICE = {".docx", ".odt", ".pptx", ".odp"}
SUPPORTED_SUFFIXES = PAGED_SUFFIXES | IMAGE_SUFFIXES | TEXT_SUFFIXES | OFFICE
...
def inspect_document(
    ...
    page with raster images over three quarters of it, a long report 3,556 characters and
    no raster at all.
    """
    source = Path(path)
    ...
    if suffix in OFFICE | TEXT_SUFFIXES:
