...
| **Download HTML** | One page, tables rendered, images inside it. Per document or all. |
...
| **Download everything as a zip** | Everything, in three folders. |

The zip holds three folders, so a batch of hundreds needs no digging:

```
markdown/     every document's text together
html/         the same, as pages, pictures referenced where they lie
documents/    the full per-document folders, manifests and images
```

The HTML in the zip points at the pictures under `documents/`, so nothing is
stored twice and it works once extracted. The **Download HTML** button gives a
different thing: one file with the pictures inside it, to send to somebody.
...
| **Slides** | `.pptx`, `.odp`, with speaker notes |
...
Two habits of official templates get handled. Heading levels are read from the
style definitions, so one language's `Naslov1`
...
raster images over three quarters of it; a long report 3,556 characters and no
raster at all. It also turns Tables on when the sample holds
...
A bordered callout box, the shape many templates use for highlights, looks
...
page; a box or a chart leaves most columns blank on most rows. Measured on a long
report, real tables ran 0 to 33% thin columns and boxes and charts 50 to 100%, so a grid with half its columns thin gets turned down, as does
one whose cells average more than 120 characters or hold a 600-character
paragraph.

On one 118-page report, 30 grids were detected and 7 kept. Pipe-table lines in
...
horizontal rules and nothing else, a common style in academic and official
reports, goes undetected: the detector reads lines, and that layout gives it
almost none.
...
long report, a chart's labels run 1.5 to 4 words a line against 9
...
Markdown by default. **Download HTML** builds one page holding every document,
on request, so no setting has to have been on beforehand. On the desktop, choose
**Markdown and HTML** or pass `--html` and a self-contained copy lands beside the
markdown: one file, tables rendered, and every
